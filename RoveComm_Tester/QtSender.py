...
class Sender(QWidget):
    ...
    def addUdpEvent(self):
        self.send_widgets.append(sendWidgetUdp(self.rovecommUdp, self.xboxCont, self, len(self.send_widgets)))
        self.redrawWidgets()

    # Handler for adding a tcp send widget
    def addTCPEvent(self):
        self.send_widgets.append(sendWidgetTCP(self.rovecommTCP, self, len(self.send_widgets)))
        self.redrawWidgets()
    ...
    def removeEvent(self, row_index):
        ...
        self.send_widgets = self.send_widgets[:row_index] + self.send_widgets[row_index + 1 :]
                ...
                for i in range(0, int(data["packet_count"])):
                    try:
                        if data["packet"][i]["protocol"] == "UDP":
                            ...
                            self.send_widgets[start_number + i].update_ms_le.setText(data["packet"][i]["update_ms"])
                            self.send_widgets[start_number + i].ip_octet_4_le.setText(data["packet"][i]["ip_octet_4"])
                            self.send_widgets[start_number + i].port_le.setText(data["packet"][i]["port"])
                            self.send_widgets[start_number + i].data_type_cb.setCurrentText(
                                data["packet"][i]["data_type"]
                            )
    ...
    def writeJSON(self):
        data = {"packet_count": len(self.send_widgets)}
        data["packet"] = []
    ...
    def sendEvent(self):
        ...
        try:
            ...
            packet = RoveCommPacket(
                int(self.data_id_le.text()),
                data_types[self.data_type_cb.currentText()],
                data,
                self.ip_octet_4_le.text(),
            )
